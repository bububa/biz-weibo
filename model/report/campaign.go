...
type CampaignDataRequest struct {
	...
	Filtering    *CampaignDataFilter `json:"filtering,omitempty"`     // 筛选条件
}

type CampaignDataFilter struct {
	CampaignIds []int64 `json:"campaign_ids"`
	Objective   []int64 `json:"objective"`
}

// URL implement Request interface
func (r CampaignDataRequest) URL() string {
	return "v3/report/effect/get_campaign_data"
}

// Payload implement Request interface
func (r CampaignDataRequest) Payload() *model.Payload {
	p := new(model.Payload)
	if r.AdvertiserID > 0 {
...
type CampaignData struct {
	List []CampaignDataMetrics `json:"list"`
	Sum  SumMetrics            `json:"sum"`
	PageInfo
}

type CampaignDataMetrics struct {
