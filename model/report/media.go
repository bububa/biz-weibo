...
type MediaDataRequest struct {
	...
	Filtering    *MediaDataFilter `json:"filtering,omitempty"`     // 筛选条件
}
...
func (r MediaDataRequest) URL() string {
	return "v3/report/effect/get_media_data"
}

// Payload implement Request interface
func (r MediaDataRequest) Payload() *model.Payload {
	p := new(model.Payload)
	if r.AdvertiserID > 0 {
...
type MediaDataResponse struct {
	model.BaseResponse
	Data MediaData `json:"data"`
}

type MediaData struct {
	List []MediaDataMetrics `json:"list"`
	Sum  Metrics            `json:"sum"`
	PageInfo
}

type MediaDataMetrics struct {
